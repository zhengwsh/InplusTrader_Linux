...
# 回测模块相关
EVENT_BKT_LOG = 'eBktLog'               # 回测相关的日志事件
EVENT_BKT_STRATEGY = 'eBktStrategy.'    # 回测策略状态变化事件
EVENT_SET_CHANGED = 'eBktSetting.'      # 回测策略参数变化事件

# 行情记录模块相关
EVENT_DATARECORDER_LOG = 'eDataRecorderLog' # 行情记录日志更新事件tick
EVENT_DATARECORDER_LOG2 = 'eDataRecorderLog2' # 行情记录日志更新事件min
EVENT_MARKETDATA = 'eMarketData'            # 行情推送事件
EVENT_MARKETDATA_CONTRACT = 'eMarketData.'  # 特定合约的行情事件
EVENT_DR_START = 'eDataRecordStart'         # 行情记录开始
