...
from ..ctaBase import *
from ..ctaTemplate1 import CtaTemplate1
import numpy as np
import pandas as pd
from statsmodels.tsa.stattools import coint

########################################################################
class SpreadTradingStrategy(CtaTemplate1):
    """期货配对交易策略Demo"""
    className = 'SpreadTradingStrategy'
    ...
    def __init__(self, ctaEngine, setting):
        """Constructor"""
        super(SpreadTradingStrategy, self).__init__(ctaEngine, setting)
    ...
    def onBar(self, bar):
        ...
        if bar.vtSymbol == self.vtSymbol:
            self.record.append(bar.close)
            self.symbolData = pd.DataFrame()
            self.symbolData['close'] = self.record
        if bar.vtSymbol == self.vtSymbol1:
            self.record1.append(bar.close)
            self.symbol1Data = pd.DataFrame()
            self.symbol1Data['close'] = self.record1
    ...
    #----------------------------------------------------------------------
    def onSpread(self):
        mean = (self.symbolData['close'] - self.symbol1Data['close']).rolling(window=self.window, center=False).mean()
        std = (self.symbolData['close'] - self.symbol1Data['close']).rolling(window=self.window, center=False).std()
        upper = mean + std / (self.Kurtosis / 3) - self.Skewness
        lower = mean - std / (self.Kurtosis / 3) - self.Skewness
        spreads = (self.symbolData['close'] - self.symbol1Data['close'])
        lastspread = 0
        spread = 0
        try:
            lastspread = spreads[len(spreads)-2]
            spread = spreads[len(spreads)-1]
        except:
            return

        if lastspread < upper[len(upper)-2] and spread > upper[len(upper)-1] and not self.up_cross_up_limit:
            self.short(self.record[-1], 1) # 卖开
            self.buy1(self.record1[-1], 1) # 买开
            self.up_cross_up_limit = True
        elif lastspread > lower[len(lower)-2] and spread < lower[len(lower)-1] and not self.down_cross_down_limit:
            self.short1(self.record1[-1], 1) # 卖开
            self.buy(self.record[-1], 1) # 买开
            self.down_cross_down_limit = True
        elif lastspread > mean[len(mean)-2] and spread < mean[len(mean)-1] and self.up_cross_up_limit:
            self.sell1(self.record1[-1], 1) # 卖平
            self.cover(self.record[-1], 1) # 买平
            self.up_cross_up_limit = False
        elif lastspread < mean[len(mean)-2] and spread > mean[len(mean)-1] and self.down_cross_down_limit:
            self.sell(self.record[-1], 1) # 卖平
            self.cover1(self.record1[-1], 1) # 买平
            self.down_cross_down_limit = False
        else:
            pass
