...
class EmaDemoStrategy(CtaTemplate):
    ...
    slowMa0 = EMPTY_FLOAT
    slowMa1 = EMPTY_FLOAT
    pos = {}  # 各合约持仓情况, 通过vtSymbol映射！！！ pos['cu1704']
    ...
    def onBar(self, bar):
        ...
        #print bar.vtSymbol + " pos long : " + str(self.pos[bar.vtSymbol]["long"]) + " pos short : " + str(self.pos[bar.vtSymbol]["short"])
        if crossOver:
            # 如果金叉时手头没有持仓，则直接做多
            if self.pos[bar.vtSymbol]["short"] == 0 : #self.pos == 0:
                self.buy(bar.vtSymbol, bar.close, 1)
            # 如果有空头持仓，则先平空，再做多
            else :  #self.pos < 0:
                self.cover(bar.vtSymbol, bar.close, 1)
                self.buy(bar.vtSymbol, bar.close, 1)
        # 死叉和金叉相反
        elif crossBelow:
            if self.pos[bar.vtSymbol]["long"] == 0 :  #self.pos == 0:
                self.short(bar.vtSymbol, bar.close, 1)
            else :   #self.pos > 0:
                self.sell(bar.vtSymbol, bar.close, 1)
                self.short(bar.vtSymbol, bar.close, 1)
