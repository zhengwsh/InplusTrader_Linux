...
class report_painter:
    ...
    def pricePaint(self):
        ...
        w_radio = (self.w-sum_width-2)/240.00 #240mins/day
...
class Test(QtGui.QWidget):
    ...
    def readSymbolData(self, symbol):
        #读取数据 mongodb
        #f = open(symbol, 'r')
        #data = f.readlines()
        #f.close()
        ...
        for row in range(data.shape[0]):
            ...
            '''
            self.stk_data['list']['time'].append(data.iloc[row]['time'])
            self.stk_data['list']['open'].append(float(data.iloc[row]['open']))
            self.stk_data['list']['high'].append(float(data.iloc[row]['high']))
            self.stk_data['list']['low'].append(float(data.iloc[row]['low']))
            self.stk_data['list']['close'].append(float(data.iloc[row]['close']))
            self.stk_data['list']['vol'].append(int(float(data.iloc[row]['volume'])))
            self.stk_data['list']['amount'].append(int(float(data.iloc[row]['amount'])))
        ...
        self.stk_data['lastclose'] = 9.14 #上一个交易日收盘价
