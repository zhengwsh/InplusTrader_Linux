...
class report_painter:
    ...
    def PriceGridPaint(self):
        ...
        #股票昨收中间线 0.00%
        self.paint.drawLine(self.grid_padding_left, grid_height*0.35+self.grid_padding_top,
    ...
    def rightGridPaint(self):
        ...
        _h+=23
        self.paint.drawLine(_x, _h, self.w, _h)
        self.paint.drawLine(_x, 23, _x, self.h-self.grid_padding_bottom)
        self.paint.drawLine(_x, self.h-self.grid_padding_bottom,
                            self.w, self.h-self.grid_padding_bottom)
    ...
    def pricePaint(self):
        ...
        w_radio = (self.w-sum_width-2)/242.00 #240mins/day
...
class Test(QtGui.QWidget):
    ...
    def compare_time(self, l_time, start_t, end_t):
        s_time = time.mktime(time.strptime('2016-01-01 %s' % start_t,'%Y-%m-%d %H:%M:%S'))
        e_time = time.mktime(time.strptime('2016-01-01 %s' % end_t,'%Y-%m-%d %H:%M:%S'))
        log_time = time.mktime(time.strptime('2016-01-01 %s' % l_time,'%Y-%m-%d %H:%M:%S'))
    ...
    def readSymbolData(self, symbol, date):
        #读取数据 mongodb
        #f = open(symbol, 'r')
        #data = f.readlines()
        #f.close()
        #data = pd.read_csv('./'+symbol+'.csv', header = 0)

        self.stk_info['name'] = Symbol_Db['equity'].find({"code":symbol[2:8]})[0]['name']
        self.stk_info['code'] = symbol[2:8]
        self.stk_info['market'] = symbol[:2]

        count = 0
        try:
            fetch_data = OneMin_Db[symbol[2:8]].find({"date":date}).sort("time",pymongo.ASCENDING)
        except:
            print u"当天无数据"
            self.has_data = False
            return
        ...
        for item in fetch_data:
            if not self.compare_time(item['time'], '09:30:00', '15:00:00'):
                continue 
            if float(item['volume']) == 0:
                continue
            #print item['time']
            count +=1 
            self.stk_data['list']['time'].append(item['time'])
            self.stk_data['list']['open'].append(float(item['open']))
            self.stk_data['list']['high'].append(float(item['high']))
            self.stk_data['list']['low'].append(float(item['low']))
            self.stk_data['list']['close'].append(float(item['close']))
            self.stk_data['list']['vol'].append(float(item['volume']))
            self.stk_data['list']['amount'].append(float(item['amount']))
        ...
        #self.stk_data['lastclose'] = 9.15 #上一个交易日收盘价
        preDate = du.last_tddate2(date)
        print  preDate
        self.stk_data['lastclose'] = float(Daily_Db[symbol[2:8]].find({"date":preDate})[0]['close'])
...
if __name__ == '__main__':
    ...
    dt = Test()
    dt.readSymbolData(sys.argv[1], sys.argv[2])
    dt.show()
    app.exec_()

    #python drawDK.py 600048 2016-01-01 2017-02-20