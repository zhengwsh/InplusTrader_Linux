...
def day_last_week2(date, days=-7):
    lasty = date + datetime.timedelta(days)
    return lasty.strftime('%Y-%m-%d')
...
def last_tddate2(date):
    date = datetime.datetime.strptime(date, '%Y-%m-%d')
    date2 = int(date.strftime("%w"))
    if date2 == 1:
        return day_last_week2(date, -3)
    else:
        return day_last_week2(date, -1)
